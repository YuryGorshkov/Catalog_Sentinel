...
<div class="gcs-page">
    ...
    <div class="gcs-cards">
        <div class="gcs-card"><span class="gcs-card-label"><?= $escape(Loc::getMessage('GCS_CURRENT_VALUE')) ?></span><span class="gcs-card-value"><?= $escape($formatPercent($currentRatio)) ?></span><span class="gcs-card-note"><?= $escape($primaryRuleLabel) ?></span></div>
        <div class="gcs-card"><span class="gcs-card-label"><?= $escape(Loc::getMessage('GCS_BASELINE_VALUE')) ?></span><span class="gcs-card-value"><?= $escape($formatPercent($baselineRatio)) ?></span><span class="gcs-card-note"><?= $escape(Loc::getMessage('GCS_BASELINE_SAMPLES', ['#COUNT#' => (string) ($primaryRule?->baseline['sample_count'] ?? 0)])) ?></span></div>
        <div class="gcs-card"><span class="gcs-card-label"><?= $escape(Loc::getMessage('GCS_AFFECTED_OBJECTS')) ?></span><span class="gcs-card-value"><?= $escape($formatNumber($affected)) ?></span><span class="gcs-card-note"><?= $escape(Loc::getMessage('GCS_OF_OBJECTS', ['#TOTAL#' => $formatNumber($objectsTotal)])) ?></span></div>
        <div class="gcs-card"><span class="gcs-card-label"><?= $escape(Loc::getMessage('GCS_FILE_CARD')) ?></span><span class="gcs-card-value"><?= $escape($scan->file->fileName) ?></span><span class="gcs-card-note"><?= $escape($formatBytes($scan->file->size)) ?> · <?= $escape($scan->documentKind) ?> · <?= $escape((string) $scan->durationMs) ?> ms</span></div>
    </div>

    <?php if ($primaryRule !== null): ?>
        <section class="gcs-section">
            <h2><?= $escape(Loc::getMessage('GCS_PRIMARY_REASON')) ?></h2>
    ...
    <?php if ($productRows !== []): ?>
        <section class="gcs-section">
            <h2><?= $escape(Loc::getMessage('GCS_AFFECTED_PRODUCTS')) ?></h2>
    ...
    <?php if ($decision === 'BLOCK'): ?>
        <section class="gcs-section"><h2><?= $escape(Loc::getMessage('GCS_NEXT_STEPS')) ?></h2><ol class="gcs-actions"><li><?= $escape(Loc::getMessage('GCS_BLOCK_ACTION_CHECK')) ?></li><li><?= $escape(Loc::getMessage('GCS_BLOCK_ACTION_FIX')) ?></li><li><?= $escape(Loc::getMessage('GCS_BLOCK_ACTION_REPEAT')) ?></li></ol></section>
    <?php endif; ?>
    ...
    <div class="gcs-buttons">
        ...
        <a class="gcs-button" href="gorshkov_catalogsentinel_export.php?id=<?= $id ?>&amp;format=json&amp;lang=<?= urlencode(LANGUAGE_ID) ?>"><?= $escape(Loc::getMessage('GCS_EXPORT_JSON')) ?></a>
        <a class="gcs-button" href="gorshkov_catalogsentinel_export.php?id=<?= $id ?>&amp;format=csv&amp;lang=<?= urlencode(LANGUAGE_ID) ?>"><?= $escape(Loc::getMessage('GCS_EXPORT_CSV')) ?></a>
        <?php if ($hasFullReport): ?><a class="gcs-button" target="_blank" rel="noopener" href="gorshkov_catalogsentinel_full_report.php?id=<?= $id ?>&amp;lang=<?= urlencode(LANGUAGE_ID) ?>"><?= $escape(Loc::getMessage('GCS_FULL_REPORT')) ?></a><?php endif; ?>
        <?php if ($scan->dryRun && $decision !== 'BLOCK' && AdminGuard::right() >= 'W'): ?><form class="gcs-inline-form" method="post"><?= bitrix_sessid_post() ?><input type="hidden" name="id" value="<?= $id ?>"><button name="accept_baseline" value="Y"><?= $escape(Loc::getMessage('GCS_ACCEPT_BASELINE')) ?></button></form><?php endif; ?>
    </div>
