...
<div class="gcs-scans">
    ...
    <form class="gcs-filter" method="get">
        ...
        <h2><?= $escape(Loc::getMessage('GCS_FILTER_HEADING')) ?></h2>
        <div class="gcs-filter-grid">
            <div class="gcs-field"><label for="gcs-status"><?= $escape(Loc::getMessage('GCS_STATUS')) ?></label><select id="gcs-status" name="status"><option value=""><?= $escape(Loc::getMessage('GCS_ALL')) ?></option><?php foreach ($statusOptions as $value): ?><option value="<?= $escape($value) ?>"<?= $filters['STATUS'] === $value ? ' selected' : '' ?>><?= $escape($statusLabel($value)) ?></option><?php endforeach; ?></select></div>
            <div class="gcs-field"><label for="gcs-decision"><?= $escape(Loc::getMessage('GCS_DECISION')) ?></label><select id="gcs-decision" name="decision"><option value=""><?= $escape(Loc::getMessage('GCS_ALL')) ?></option><?php foreach ($decisionOptions as $value): ?><option value="<?= $escape($value) ?>"<?= $filters['DECISION'] === $value ? ' selected' : '' ?>><?= $escape($decisionLabel($value)) ?></option><?php endforeach; ?></select></div>
            <div class="gcs-field"><label for="gcs-kind"><?= $escape(Loc::getMessage('GCS_KIND')) ?></label><select id="gcs-kind" name="document_kind"><option value=""><?= $escape(Loc::getMessage('GCS_ALL')) ?></option><?php foreach ($kindOptions as $value): ?><option value="<?= $escape($value) ?>"<?= $filters['DOCUMENT_KIND'] === $value ? ' selected' : '' ?>><?= $escape($kindLabel($value)) ?></option><?php endforeach; ?></select></div>
            <div class="gcs-field"><label for="gcs-source"><?= $escape(Loc::getMessage('GCS_SOURCE')) ?></label><select id="gcs-source" name="source_key"><option value=""><?= $escape(Loc::getMessage('GCS_ALL')) ?></option><?php foreach ($sources as $key => $label): ?><option value="<?= $escape($key) ?>"<?= $filters['SOURCE_KEY'] === $key ? ' selected' : '' ?>><?= $escape($label) ?></option><?php endforeach; ?></select></div>
            <div class="gcs-field"><label for="gcs-rule"><?= $escape(Loc::getMessage('GCS_FILTER_RULE')) ?></label><select id="gcs-rule" name="rule_code"><option value=""><?= $escape(Loc::getMessage('GCS_ALL')) ?></option><?php foreach ($ruleOptions as $value): ?><option value="<?= $escape($value) ?>"<?= $filters['RULE_CODE'] === $value ? ' selected' : '' ?>><?= $escape($ruleLabel($value)) ?></option><?php endforeach; ?></select></div>
            <div class="gcs-field"><label for="gcs-run-kind"><?= $escape(Loc::getMessage('GCS_FILTER_RUN_KIND')) ?></label><select id="gcs-run-kind" name="is_dry_run"><option value=""><?= $escape(Loc::getMessage('GCS_ALL')) ?></option><option value="N"<?= $filters['IS_DRY_RUN'] === 'N' ? ' selected' : '' ?>><?= $escape(Loc::getMessage('GCS_RUN_AUTOMATIC')) ?></option><option value="Y"<?= $filters['IS_DRY_RUN'] === 'Y' ? ' selected' : '' ?>><?= $escape(Loc::getMessage('GCS_RUN_MANUAL')) ?></option></select></div>
            <div class="gcs-field"><label for="gcs-date-from"><?= $escape(Loc::getMessage('GCS_DATE_FROM')) ?></label><input id="gcs-date-from" type="date" name="date_from" value="<?= $escape($filters['DATE_FROM']) ?>"></div>
            <div class="gcs-field"><label for="gcs-date-to"><?= $escape(Loc::getMessage('GCS_DATE_TO')) ?></label><input id="gcs-date-to" type="date" name="date_to" value="<?= $escape($filters['DATE_TO']) ?>"></div>
        </div>
